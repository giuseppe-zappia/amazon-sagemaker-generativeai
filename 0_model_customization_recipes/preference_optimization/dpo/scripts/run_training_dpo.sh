...
export FI_PROVIDER=$(yq -r ".fi_provider" $training_recipe)
export NCCL_PROTO=$(yq -r ".nccl_proto" $training_recipe)
export NCCL_SOCKET_IFNAME=$(yq -r ".nccl_socket_ifname" $training_recipe)
export NCCL_IB_DISABLE=$(yq -r ".nccl_ib_disable" $training_recipe)
export NCCL_DEBUG=$(yq -r ".nccl_debug" $training_recipe)
